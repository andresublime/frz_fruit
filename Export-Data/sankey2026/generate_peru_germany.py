#!/usr/bin/env python3
"""Generate Sankey diagrams for Peru exports to Germany"""
import sys
...
def main():
    """Generate Peru to Germany sankey diagram"""
    DB_PATH = "../data/exports.db"
    OUTPUT_DIR = Path(__file__).parent / "peru" / "germany"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print("="*60)
    print("Generating Peru → Germany Sankey Diagram")
    print("="*60)
    ...
    # Filter to Peru and Germany only
    ...
    df_peru_germany = df_peru[df_peru['Country'] == 'Germany'].copy()

    print(f"Peru exports to Germany: {len(df_peru_germany)} records")
    print(f"Total MT: {df_peru_germany['MT'].sum():.0f}")

    if len(df_peru_germany) == 0:
        print("No Peru exports to Germany found!")
        sys.exit(1)

    # Show fruit breakdown
    print("\nFruit breakdown:")
    fruit_totals = df_peru_germany.groupby('Fruit')['MT'].sum().sort_values(ascending=False)
    ...
    # Write mermaid file
    mmd_file = OUTPUT_DIR / "sankey-peru-germany-all-fruits.mmd"
    mmd_file.write_text(output)
    print(f"\n✓ Mermaid file: {mmd_file}")

    # Generate PNG using mmdc
    png_file = OUTPUT_DIR / "sankey-peru-germany-all-fruits.png"
    ...
    print("\n" + "="*60)
    print("✓ Peru → Germany Sankey complete!")
    print("="*60)
