#!/usr/bin/env python3
"""Generate individual Sankey diagrams for each fruit from Ecuador to Europe"""
import sys
...
def main():
    """Generate Ecuador to Europe sankey diagrams per fruit"""
    DB_PATH = "../data/exports.db"
    OUTPUT_DIR = Path(__file__).parent / "ecuador" / "europe"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print("="*60)
    print("Generating Ecuador → Europe Sankey Diagrams PER FRUIT")
    print("="*60)
    ...
    for fruit_name in top_fruits:
        ...
        df_fruit = df_ecuador[df_ecuador['Fruit'] == fruit_name].copy()
        ...
        # Generate sankey (skip fruit level since it's only one fruit)
        # Use min_value to filter small flows + moderate "Others" aggregation
        ...
        output = create_supply_chain_sankey(
            df_fruit,
            value_col='MT',
            min_value=min_threshold,  # Filter out small flows
            aggregate_others=True,
            others_percentage=0.30,  # 30% to Others
            font_size=18
        )
        ...
        mmd_file = OUTPUT_DIR / f"sankey-ecuador-europe-{fruit_key}.mmd"
        ...
        # Generate PNG
        png_file = OUTPUT_DIR / f"sankey-ecuador-europe-{fruit_key}.png"
    ...
    print("\n" + "="*60)
    print(f"✓ Generated {len(top_fruits)} Ecuador → Europe per-fruit diagrams!")
    print("="*60)
