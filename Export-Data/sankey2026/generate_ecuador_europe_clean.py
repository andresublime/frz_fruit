#!/usr/bin/env python3
"""Generate CLEAN Sankey diagram for Ecuador exports to Europe with aggressive aggregation"""
import sys
...
def main():
    """Generate Ecuador to Europe sankey diagram with aggressive filtering"""
    DB_PATH = "../data/exports.db"
    OUTPUT_DIR = Path(__file__).parent / "ecuador" / "europe"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print("="*60)
    print("Generating Ecuador → Europe Sankey Diagram (CLEAN)")
    print("="*60)

    # Read Ecuador exports to Europe
    ...
    print(f"Ecuador exports to Europe: {len(df_ecuador)} records")
    print(f"Total MT: {df_ecuador['MT'].sum():.0f}")

    if len(df_ecuador) == 0:
        print("No Ecuador exports to Europe found!")
        sys.exit(1)

    # Show fruit breakdown
    print("\nFruit breakdown:")
    fruit_totals = df_ecuador.groupby('Fruit')['MT'].sum().sort_values(ascending=False)
    for fruit, mt in fruit_totals.items():
        print(f"  {fruit}: {mt:.0f} MT")

    # Generate sankey with AGGRESSIVE aggregation
    print("\nGenerating Exporter → Fruit → Importer → Country sankey...")
    print("Using 50% aggregation threshold (top 50% = individual, bottom 50% = 'Others')")
    output = create_supply_chain_sankey(
        df_ecuador,
        value_col='MT',
        aggregate_others=True,
        others_percentage=0.50,  # MUCH more aggressive - only top 50% shown individually
        font_size=18
    )
    ...
    # Write mermaid file
    mmd_file = OUTPUT_DIR / "sankey-ecuador-europe-all-fruits-clean.mmd"
    mmd_file.write_text(output)

    # Count flows
    flow_count = output.count('\n') - 12  # Subtract header lines
    print(f"\n✓ Mermaid file: {mmd_file}")
    print(f"  Flow count: {flow_count}")

    # Generate PNG using mmdc
    png_file = OUTPUT_DIR / "sankey-ecuador-europe-all-fruits-clean.png"
    print(f"\nGenerating PNG: {png_file}...")
    ...
    print("\n" + "="*60)
    print("✓ Ecuador → Europe CLEAN Sankey complete!")
    print("="*60)
